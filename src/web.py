...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header h1 { font-size: 24px; color: #0ff; }
    ...
    <div class="container">
        <div class="grid">
            ...
            <div class="card">
                ...
                <table>
                    <thead>
                        <tr><th>目标</th><th>平均延迟</th><th>抖动</th><th>范围</th></tr>
                    </thead>
        ...
        <div class="card" style="margin-bottom: 20px;">
            ...
            <table>
                <thead>
                    <tr><th>目标</th><th>平均延迟</th><th>最低</th><th>最高</th><th>丢包</th><th>样本数</th></tr>
                </thead>
        ...
        <div class="card">
            ...
            <table>
                <thead>
                    <tr><th>时间</th><th>类型</th><th>级别</th><th>消息</th></tr>
                </thead>
    ...
    <script>
        ...
        function initCharts() {
            ...
            latencyChart = new Chart(document.getElementById('latencyChart'), {
                ...
                options: { ...opts, scales: { ...opts.scales, y: { ...opts.scales.y, title: { display: true, text: 'ms', color: '#888' } } } }
            });

            lossChart = new Chart(document.getElementById('lossChart'), {
                type: 'line',
                data: { labels: [], datasets: [] },
                options: { ...opts, scales: { ...opts.scales, y: { ...opts.scales.y, title: { display: true, text: '%', color: '#888' } } } }
            });

            speedChart = new Chart(document.getElementById('speedChart'), {
                type: 'line',
                data: { labels: [], datasets: [] },
                options: { ...opts, scales: { ...opts.scales, y: { ...opts.scales.y, title: { display: true, text: 'Mbps', color: '#888' } } } }
            });
        }
        ...
        async function loadStatus() {
            ...
            document.getElementById('status-table').innerHTML = data.map(row => `
                <tr>
                    <td>${row.name}</td>
        ...
        async function loadChart(period) {
            ...
            if (latencyData && latencyData.timestamps) {
                latencyChart.data = {
                    ...
                    datasets: [{ label: '延迟', data: latencyData.values, borderColor: '#0ff', backgroundColor: 'rgba(0,255,255,0.1)', fill: true }]
                };
                latencyChart.update();
            }
            ...
            if (lossData && lossData.timestamps) {
                lossChart.data = {
                    ...
                    datasets: [{ label: '丢包', data: lossData.values, borderColor: '#f44', backgroundColor: 'rgba(255,68,68,0.1)', fill: true }]
                };
                lossChart.update();
            }
            ...
            if (speedData && speedData.timestamps) {
                speedChart.data = {
                    ...
                    datasets: [{ label: '带宽', data: speedData.values, borderColor: '#0f0', backgroundColor: 'rgba(0,255,0,0.1)', fill: true }]
                };
                speedChart.update();
            }
        }
        ...
        async function refreshAll() {
            await Promise.all([loadServerInfo(), loadStatus(), loadBaselines(), loadAlerts()]);
        }
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        handlers = {
            '/api/server': lambda: self._handle_server_info(),
            '/api/status': lambda: self._handle_status(query),
            '/api/config': lambda: self._handle_config(),
            '/api/baselines': lambda: self._handle_baselines(),
            '/api/alerts': lambda: self._handle_alerts(query),
            '/api/chart/speed': lambda: self._handle_chart_speed(query),
        }
    ...
    def _handle_alerts(self, query):
        ...
        self._send_json([{'timestamp': r[0], 'alert_type': r[1], 'severity': r[2], 'message': r[3]} for r in rows])
    ...
    def _handle_chart_latency(self, path, query):
        ...
        if len(rows) > 500:
            ...
            rows = [(sum(r[0] for r in rows[i:i+bucket_size]) / bucket_size,
                     sum(r[1] for r in rows[i:i+bucket_size]) / bucket_size)
                    for i in range(0, len(rows), bucket_size)]
        self._send_json({'timestamps': [r[0] for r in rows], 'values': [round(r[1], 1) for r in rows]})
    ...
    def _handle_speed_run(self):
        try:
            ...
            if result['success']:
                _db.save_speed(url, result['speed_mbps'], result['downloaded_bytes'], result['elapsed_sec'], False)
            self._send_json({
